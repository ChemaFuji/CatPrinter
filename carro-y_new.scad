...
module yc_holes(){
	...
	cylinder(r=bearing_diameter/2,h=100,$fn=20);
		...
		//huecos para poleas
	translate(v=[0,-yc_ancho/2-4-9,10]) 
	 cube(size = [belt_space,30,22], center = true);
	translate(v=[0,-yc_ancho/2-4-9,yc_largo-10]) 
	 cube(size = [belt_space,30,22], center = true);
...
//			cube([yc_ancho,5,4], center=true);
	...
	//screws poleas 
	translate ([40,-28.5,(yc_largo-40)/2]) rotate ([0,-90,0]) 
	 screw(r=m3_diameter/2+0.1, slant=false, head_drop=32, h=100, $fn=10);
	translate ([20,-28.5,yc_largo-(yc_largo-40)/2]) rotate ([0,-90,0]) 
	 screw(r=m3_diameter/2+0.1, slant=false, head_drop=10, h=50, $fn=10);
	
	translate ([-2-15,-28.5,(yc_largo-40)/2]) rotate ([180,-90,0]) 
	 screw(r=m3_diameter/2+0.1, slant=false, head_drop=4+5, h=50, $fn=6);
	translate ([-2-20,-28.5,yc_largo-(yc_largo-40)/2]) rotate ([180,-90,0]) 
